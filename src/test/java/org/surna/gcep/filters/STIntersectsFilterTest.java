 ...
 */
public class STIntersectsFilterTest
{
	private GeometryFactory geometryFactory;
	private Geometry testArea;             // polygon which is a linear ring
	private Point pointIntersects;         // point is implemented as two coordinates
	private Point pointNoIntersect;
	private Geometry polyIntersects;
	private Geometry polyNoIntersect;
	private Geometry lineIntersects;        // jts implements a line as a linestring, which is like a polygon
	...
	public void setUpBeforeClass() {
		...
		polyIntersects = geometryFactory.createPolygon(shell1, null);
		...
		pointIntersects = geometryFactory.createPoint(coordinate);  
		/*
		...
		lineIntersects = new GeometryFactory().createLineString(l1);
	...
	{
		STIntersectsFilter test = STIntersectsFilter.getInstance();
		Assert.assertNotNull(test);
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: Point inside the testArea. Tests as an intersect. If you make a geometry out of the intersect,
	 * it turns out to be a point with coordinates pointIntersects.
	 */
	@Test
	public void testSTIntersectsWithPointIntersects()
	{
		Assert.assertTrue(STIntersectsFilter.getInstance().STIntersects(testArea, pointIntersects));
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: Point outside the testArea means it does not intersect.  If you contstruct a 
	 * geometry of the intersection, you get an empty geometry collection.
	 */
	@Test
	public void testSTIntersectsNoIntersectWithPoint()
	{
		Assert.assertFalse(STIntersectsFilter.getInstance().STIntersects(testArea, pointNoIntersect));		
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: Polygon inside the testArea means it intersects
	 */
	@Test
	public void testSTIntersectsWithPolyIntersects()
	{
		Assert.assertTrue(STIntersectsFilter.getInstance().STIntersects(testArea, polyIntersects));
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: Polygon outside the testArea means it does not intersect
	 */
	@Test
	public void testSTIntersectsNoIntersectWithPoly()
	{
		Assert.assertFalse(STIntersectsFilter.getInstance().STIntersects(testArea, polyNoIntersect));
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: line starts outside the testArea and goes inside testArea means it intersects
	 */
	@Test
	public void testSTIntersectsIntersectWithLine()
	{
		Assert.assertTrue(STIntersectsFilter.getInstance().STIntersects(testArea, lineIntersects));
	}

	/**
	 * Test method for {@link org.surna.gcep.filters.STIntersectsFilter#STIntersects(com.vividsolutions.jts.geom.Geometry, com.vividsolutions.jts.geom.Geometry)}.
	 * Boundry condition: line starts outside the testArea and does not go inside testArea means it does not intersect
	 */
	@Test
	public void testSTIntersectsNoIntersectWithLine()
	{
		Assert.assertFalse(STIntersectsFilter.getInstance().STIntersects(testArea, lineNoIntersect));
	}
